...
function JS_DevGetNetworkportSettingType()
{
	var lTmpVar;
	if (s_DeviceXMLDom != null)
	{
		lTmpVar = s_DeviceXMLDom.getElementsByTagName("network_port");
		if (lTmpVar.length > 0)
		{
			lTmpVar = lTmpVar[0];
			return JS_XMLGetTagValue(lTmpVar, "network_port");
		}
	}
	return "AUTO";
}
...
function JS_DevGetTimingSetting()
{
    var lTmpVar;
    var lItemInfo = new Object();
    if (s_DeviceXMLDom != null)
    {
        lTmpVar = s_DeviceXMLDom.getElementsByTagName("timing_setting");
        if (lTmpVar.length > 0)
        {
            lTmpVar = lTmpVar[0];
            lItemInfo.m_TimingInterval = parseInt(JS_XMLGetTagValue(lTmpVar, "timing_interval"), 10);
            lItemInfo.m_TimingMark = JS_XMLGetTagValue(lTmpVar, "timing_mark");
        }
    }
    return lItemInfo;
}
