...
/* AJAX(XML HTTP)相关                                                             */
/************************************************************************/
function JS_LCreateXMLHttpRequest()//创建XMLHttpRequest对象       
{
	var XMLHttpReq;
...
function JS_HttpGet(Target, Parameter, BackFunction, bSync)//发送Get请求函数
{
	...
	var lXMLHttpReq = JS_LCreateXMLHttpRequest();
	...
	lXMLHttpReq.onreadystatechange = function()
	{
		if (lXMLHttpReq.readyState == 4)  // 判断对象状态
		{
			if (lXMLHttpReq.status == 200) // 信息已经成功返回，开始处理信息
			{
				...
				{
					if (JS_ISValidObject(lXMLHttpReq.responseXML))
					{
...
/* XML DOM处理相关                                                      */
/************************************************************************/
function JS_XMLSerialize(XMLDom)
{
	var lRet = null;
	try //Internet Explorer 5.5+
	{
		...
		{
			lRet = (new XMLSerializer()).serializeToString(XMLDom);
		}
	}
	catch (e)
	{
...
function JS_XMLCreateDomFromFile(XMLPathName)
{
	...
	if (XMLPathName == null)
	{
		...
		lXMLDom.async = false;
		lXMLDom.appendChild(lXMLDom.createProcessingInstruction("xml", 'version="1.0" encoding="utf-8"'));
	}
	else
	{
		var lRandom = Math.round(Math.random() * 10000);
		var xhr = window.XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject("Microsoft.XMLHTTP");
		xhr.open("GET", XMLPathName + "?" + lRandom, false);
		xhr.send(null);
		lXMLDom = xhr.responseXML;
	}
	return lXMLDom;
}
...
function JS_XMLGetValueINT(OBJ, Radix)
{
    var lTmpVar;
    lTmpVar = JS_XMLGetValue(OBJ);
    if (lTmpVar != null)
    {
...
function JS_XMLGetValueFloat(OBJ)
{
    var lTmpVar;
    lTmpVar = JS_XMLGetValue(OBJ);
    if (lTmpVar != null)
...
function JS_HTMLGetOBJByIDInd(id, Ind) {
    return $(JS_ID(id, Ind));
}


function JS_HTMLGetValueByIDAndIndFloat(ID, Ind)
{
	return JS_HTMLGetValueByIDFloat(JS_ID(ID, Ind));
}

function JS_HTMLGetValueByIDAndIndInt(ID, Ind, Radix)
{
	return JS_HTMLGetValueByIDInt(JS_ID(ID, Ind), Radix);
}

function JS_HTMLGetValueByIDAndInd(ID, Ind)
{
	return JS_HTMLGetValueByID(JS_ID(ID, Ind));
}

function JS_HTMLGetValueByIDFloat(ID)
{
	var lTmpNode;
	lTmpNode = $(ID);
	return parseFloat(JS_HTMLGetValue(lTmpNode));
}

function JS_HTMLGetValueByIDInt(ID, Radix)
{
	var lTmpNode;
	lTmpNode = $(ID);
	return parseInt(JS_HTMLGetValue(lTmpNode), Radix);
}
...
function JS_HTMLSetValue(Obj, value)//用于取得 input，select value属性
{
	var lTmpNode;
	lTmpNode = Obj;
	if (lTmpNode != null)
	{
		if (lTmpNode.nodeName.toLowerCase() == "select")
		{
			return JS_UISelectSetCurIndexByValue(lTmpNode, value, false);
		}
		else if (lTmpNode.nodeName.toLowerCase() == "input")
		{
			if (lTmpNode.type.toLowerCase() == "checkbox")
			{
				if (value > 0 || value == "ON")
				{
					lTmpNode.checked = true;
				}
				else
				{
					lTmpNode.checked = false;
				}
			}
		}
		lTmpNode.value = value;
	}
}
...
/* UI处理相关 */
function JS_UISpanCreate(Width, Txt)
{
	var lTmpVar;
	lTmpVar = document.createElement("span");
	lTmpVar.innerHtml = Txt;
	lTmpVar.style.textAlign = "center";
	lTmpVar.style.height = "18px";
	lTmpVar.style.width = Width;
	return lTmpVar;
} 

function JS_UIInputCreate(Type)
{
	var lTmpVar;
	lTmpVar = document.createElement("input");
	lTmpVar.type = Type;
	return lTmpVar;
}

function JS_UIInputTextCreate(Width)
{
	var lTmpVar;
	lTmpVar = JS_UIInputCreate("text");;
	lTmpVar.style.textAlign = "center";
	lTmpVar.style.height = "18px";
	lTmpVar.style.width = Width;
	return lTmpVar;
...
{
	var lTmpVar;
	lTmpVar = document.createElement("select");
	lTmpVar.style.height = "18px";
	return lTmpVar;
}
...
function JS_UISelectAddOptionBOOL(SelectObj, bONOFF)
{
	if (bONOFF)
	{
		JS_UISelectAddOption(SelectObj, "ON", "ON");
		JS_UISelectAddOption(SelectObj, "OFF", "OFF");
	}
	else
	{
		JS_UISelectAddOption(SelectObj, 1, "ON");
		JS_UISelectAddOption(SelectObj, 0, "OFF");
	}
}

function JS_UISelectAddOptionINT(SelectObj, Start, Step, Max, bIndex)
{
	var i;
	for (i = Start; i <= Max; i+=Step)
	{
		if (bIndex)
		{
			JS_UISelectAddOption(SelectObj, i - 1, i);
		}
		else
		{
			JS_UISelectAddOption(SelectObj, i, i);
		}
	}
}
...
function JS_UISelectSetCurIndexByValue(SelectObj, Value, bDefault)
{
	...
	if (lSys.chrome) 
	{
		for (i = 0; i < SelectObj.length; i++)
		{
			...
			{
				SelectObj.options[i].defaultSelected = false;
			}
		}
	}
	else
...
function JS_UISelectGetCurTxt(SelectObj)
{
...
function JS_UISelectCreateMarkSelect(Mark, Height)
{
	var lSelect;
	lSelect = JS_UISelectCreate();
	lSelect.style.height = Height + "px";
	JS_UISelectAddOption(lSelect, "OFF", "OFF");
	JS_UISelectAddOption(lSelect, "ON", "ON");
	JS_UISelectSetCurIndexByValue(lSelect, Mark, true);
	return lSelect;
}

function JS_UISelectCreateRSSelect(RSInd)
{
	var lSelect;
	lSelect = JS_UISelectCreate();
	lSelect.style.height = "18px";
	for (i = 0; i < 7; i++)
	{
		JS_UISelectAddOption(lSelect, i, JS_MPEG2RSIndToStr(i));
	}
	JS_UISelectSetCurIndexByValue(lSelect, RSInd, true);
	return lSelect;
}

function JS_UISelectCreateRSSelectInnerHTML(RSInd)
{
	return JS_UISelectCreateRSSelect(RSInd).innerHTML;
}
...
function JS_UIIMASetICONByID(ID, Name)
{
	var lTmpVar = null;
	lTmpVar = $(ID);
	if (lTmpVar != null)
	{
		if (lTmpVar.name != Name)
		{
			if (Name == "red")
			{
				lTmpVar.src = "graphics/red.gif";
			}
			if (Name == "green")
			{
				lTmpVar.src = "graphics/green.gif";
			}
			if (Name == "grey")
			{
				lTmpVar.src = "graphics/grey.gif";
			}
			if (Name == "yellow")
			{
				lTmpVar.src = "graphics/yellow.gif";
			}
			lTmpVar.name = Name;
		}
	}
...
//未测试()[]{}"'\
//可以作为密码~@$%*.`/|:;_
function JS_RestrctInputPASWORD(Obj)
{
	if (JS_CheckBrowser() == "IE")
	{
		Obj.onkeypress = function ()
		{
			return JS_RestrictInput(this, /^[0-9a-zA-Z~@$%*.`/|:;_]*$/, String.fromCharCode(event.keyCode));
		}
		Obj.onpaste = function ()
		{
			return JS_RestrictInput(this, /^[0-9a-zA-Z~@$%*.`/|:;_]*$/, window.clipboardData.getData('Text'));
		}
		Obj.ondrop = function ()
		{
			return JS_RestrictInput(this, /^[0-9a-zA-Z~@$%*.`/|:;_]*$/, event.dataTransfer.getData('Text'));
		}
	}
	else
	{
...
function JS_ISUndefined(Object)
{
