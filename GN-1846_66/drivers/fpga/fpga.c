...
static void FPGA_ReadTasklet(unsigned long Data)
{
	...
	while(s_FpgeDevice.force_mark)
	{
		if ((kfifo_len(s_FpgeDevice.r_fifo) + FPGA_IO_BUFF_SIZE) <= FPGA_DRIVER_KERNEL_R_FIFO_SIZE)//判断是否可写入到FIFO中！确保还剩下一个完整的缓冲区块
		{
			...
			lData = ioread16(plPHYAddr);
			plPHYAddr += 2;
			s_FpgeDevice.r_buf[0] = (*(unsigned char*)&lData);//先读取协议头4个字节，以得到长度
			s_FpgeDevice.r_buf[1] = (*((unsigned char*)&lData + 1) );

			if (s_FpgeDevice.r_buf[0] != 0)
			{
				lData = ioread16(plPHYAddr);
				plPHYAddr += 2;
				s_FpgeDevice.r_buf[2] = (*(unsigned char*)&lData);
				s_FpgeDevice.r_buf[3] = (*((unsigned char*)&lData + 1) );
				...
				if ((lPayloadSize >= 0) && (lPayloadSize <= FPGA_IO_MAX_PAYLOAD_SIZE))
				{
					...
					while(lLeft > 0)
					{
						...
						s_FpgeDevice.r_buf[i]=(*(unsigned char*)&lData);
						s_FpgeDevice.r_buf[i + 1]=(*((unsigned char*)&lData + 1));
						i += 2;
						lLeft -= 2;
					}
...
static __init int FPGA_Initiate(void)
{
	...
	/*初始化物理接口*/
	if((s_FpgeDevice.base = ioremap(FPGA_BASE_ADDRESS , SZ_64K) ) == NULL )
	{
		lRet=-EFAULT;
		return lRet;
	}

	printk("Map ARM Base Address = 0x%08X\n", FPGA_BASE_ADDRESS);
		...
		else
		{
			...
			if ((lPayloadSize >= 0) && (lPayloadSize <= FPGA_IO_MAX_PAYLOAD_SIZE))
			{
				if ((count - 4) == lPayloadSize)//保证写入的数据符合协议！
				{
					...
					while(lLeft)
					{
						lData= *((unsigned short *)(&s_FpgeDevice.w_buf[i]));
						iowrite16(lData, plPHYAddr);
						plPHYAddr += 2;
						lLeft-= 2;
						i+= 2;
					}
