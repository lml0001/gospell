...
MyListControl.prototype.UpdateListFrame = function()
{
	...
	if (lTargetItem != null)
	{
		var lOutterDIV, lInnerDIV, lTitleUL, lDataUL;
		lOutterDIV = document.createElement("div");
		lOutterDIV.className = "css_" + this.Object;
		lOutterDIV.id = this.Object;
		lOutterDIV.style.height = this.div.height + "px";
		lOutterDIV.style.width = this.div.width + "px";
		lOutterDIV.style.align = "left";
		lOutterDIV.style.textAlign = "left";
		lOutterDIV.style.overflow = "auto";
		//lOutterDIV.style.border = "thin solid #0000FF";
		
		lInnerDIV = document.createElement("div");
		lInnerDIV.style.height = (this.div.height - this.head.height - 10) + "px";
		lInnerDIV.style.width = (this.div.width - 10) + "px";
		lInnerDIV.style.overflow = "auto";
		//lInnerDIV.style.border = "thin solid #FF0000";
		
		lTitleUL = document.createElement("ul");
		lTitleUL.id = this.Object + "_title";
		
		lDataUL = document.createElement("ul");
		lDataUL.className = this.Object;
		lDataUL.id = this.Object + "_data";
		
		
		lInnerDIV.appendChild(lDataUL);
		lOutterDIV.appendChild(lTitleUL);
		lOutterDIV.appendChild(lInnerDIV);
		lTargetItem.appendChild(lOutterDIV);
...
MyListControl.prototype.AddRowToDom = function(bHead, RowData, bScrollToView, OnMouseOver, OnMouseOut)
{
	...
	if (lULNode != null)
	{
		...
		for (i = 0; i < this.ColWidthArray.length; i++)
		{
			...
			if (JS_ISValidObject(this.ColValueArray[i]) == false)
			{
