...
void MULTL_GenerateInfoXML(MULT_Handle *pHandle)
{
...
#if defined(GN1846) || defined(GN1866B)
	MULTL_XMLAddNodeUINT(plXMLRoot, "board_type", plInfo->m_InputBoardType);
#endif
#if defined(GN1866B)
	MULTL_XMLAddNodeHEX(plXMLRoot, "lt6911c_softversion", plInfo->m_pLt6911cSoftVersion);
#endif
	/*OEM信息*/
	...
	{
		CHAR_T plTmpChar[1024];
		GLOBAL_SPRINTF((plTmpChar, "M%d ", plInfo->m_LicenseMode + 1));
		if (plInfo->m_LicenseAAC)
		{
			GLOBAL_STRCAT(plTmpChar, "AA ");
			MULTL_XMLAddNodeUINT(plXMLRoot, "license_aac", 1);
		}
		if (plInfo->m_LicenseAC3)
		{
			GLOBAL_STRCAT(plTmpChar, "AC ");
			MULTL_XMLAddNodeUINT(plXMLRoot, "license_ac3", 1);
		}
		if (plInfo->m_LicenseHEVC)
		{
			GLOBAL_STRCAT(plTmpChar, "HEVC ");
			MULTL_XMLAddNodeUINT(plXMLRoot, "license_hevc", 1);
		}
		MULTL_XMLAddNodeText(plXMLRoot, "license_mode", plTmpChar);
	}
#else
	MULTL_XMLAddNodeUINT(plXMLRoot, "license_mode", plInfo->m_LicenseMode);
#endif
	...
	MULTL_XMLAddNodeINT(plXMLRoot, "fpga_sw_slot", FPGASW_WarpGetSlot());
#endif

#if defined(CPU_DATA_PORT)
	MULTL_XMLAddNodeMark(plXMLRoot, "cpu_data_port", TRUE);
#endif

#if defined(SUPPORT_WATER_MARK)
	MULTL_XMLAddNodeMark(plXMLRoot, "wm_support", TRUE);
#endif

#if defined(SUPPORT_MOSIAC)
	MULTL_XMLAddNodeMark(plXMLRoot, "mosiac_support", TRUE);
#endif
	{
		...
		if (plFile != NULL)
		{
			...
			plFile = NULL;
//            GLOBAL_TRACE((" ====>>> Test by xiaoli : Save device info xml [%s] ! \n" , plPathStr));
		}
//		else {
//            GLOBAL_TRACE((" ====>>> Test by xiaoli : Save device info xml fail ! \n"));
//		}
	}
...
void MULTL_DefaultManageIP(MULT_Handle *pHandle)
{
	...
	{
		plConfig->m_ManageIPv4Addr = MULTI_DEFAULT_IP_ADDR + FPGASW_WarpGetSlot() * 10;
	}
	else
	{
...
// Timing Apply Parameter Start
    plConfig->m_TimingInterval = 6*3600;
    plConfig->m_TimingMark = FALSE;
...
// Timing Apply Parameter Start
    plXMLHolder = mxmlNewElement(plXMLRoot, "timing_setting");
    MULTL_XMLAddNodeINT(plXMLHolder, "timing_interval", plConfig->m_TimingInterval);
    MULTL_XMLAddNodeText(plXMLHolder, "timing_mark", MULTL_XMLMarkValueToStr(plConfig->m_TimingMark));
// Timing Apply Parameter End
...
// Timing Apply Parameter Start
                            plXMLHolder = mxmlFindElement(plXMLRoot, plXMLRoot, "timing_setting", NULL, NULL, MXML_DESCEND_FIRST);
                            if (plXMLHolder)
                            {
                                plTmpStr = MULTL_XMLGetNodeText(plXMLHolder, "timing_interval");
                                if (plTmpStr)
                                {
                                    ...
                                    plMonitor->m_TimingLastDuration = 0;
                                }

                                plTmpStr = MULTL_XMLGetNodeText(plXMLHolder, "timing_mark");
                                if (plTmpStr)
                                {
                                    plConfig->m_TimingMark = MULTL_XMLMarkValueFromStr(plTmpStr);
                                }

								GLOBAL_TRACE((" =====>>> Load Timing Interval = %d .\n" , plConfig->m_TimingInterval));
								GLOBAL_TRACE((" =====>>> Load Timing Mark = %d .\n" , plConfig->m_TimingMark));
                            }
                            else
                            {
                                GLOBAL_TRACE(("Error No Timing Setting Data In XML!!!!!!!!!!!!!!!!\n"));
                            }
// Timing Apply Parameter End
		...
		case HWL_CHANNEL_TYPE_IP_DEP:
			{
				...
				if (FPGASW_WarpGetSlot() != GLOBAL_INVALID_INDEX)
				{
					plChnNode->m_ChannelInfo.m_IPInfo.m_IPAddress = MULTI_DEFAULT_IP_ADDR + FPGASW_WarpGetSlot() * 10 + lIPCurCount;
				}
				else
				{
...
void MULTL_DefaultParameter(MULT_Handle *pHandle, HWL_HWInfo* pHWInfo)
{
	...
	for (i = 0; i < plSystemParam->m_InChannelNumber + plSystemParam->m_OutChannelNumber; i++)
	{
		...
		case HWL_CHANNEL_TYPE_IP_DEP:
			{
				lAllETHCount++;
				plChnNode->m_ChannelInfo.m_IPInfo.m_IPAddress = MULTI_DEFAULT_IP_ADDR + lIPCurCount + 100;
	...
	for (i = 0; i < pHWInfo->m_ChannelNum; i++)
	{
		...
		case HWL_CHANNEL_TYPE_IP_DEP:
			...
			case HWL_CHANNEL_TYPE_ENCODER:
			{
				...
				{
					plChnNode->m_ChannelInfo.m_ENCODER.m_VidOutBitrateLimitLow = 500;
					plChnNode->m_ChannelInfo.m_ENCODER.m_VidOutBitrateLimitHigh = 30000;
					plChnNode->m_ChannelInfo.m_ENCODER.m_ProgBitrateLimitHigh = 38000;
...
#if defined(SUPPORT_WATER_MARK)
					plChnNode->m_ChannelInfo.m_ENCODER.m_x_coordinateLimitLow = 0;
					plChnNode->m_ChannelInfo.m_ENCODER.m_x_coordinateLimitHigh = 1920;

					plChnNode->m_ChannelInfo.m_ENCODER.m_y_coordinateLimitLow = 0;
					plChnNode->m_ChannelInfo.m_ENCODER.m_y_coordinateLimitHigh = 1080;

					plChnNode->m_ChannelInfo.m_ENCODER.m_transparentLimitLow = 0;
					plChnNode->m_ChannelInfo.m_ENCODER.m_transparentLimitHigh = 100;
#endif
#if defined(SUPPORT_MOSIAC)
					/* Mosaic */
					plChnNode->m_ChannelInfo.m_ENCODER.m_Mosaic_x_coordinateLimitLow = 0;
					plChnNode->m_ChannelInfo.m_ENCODER.m_Mosaic_x_coordinateLimitHigh = 1920;

					plChnNode->m_ChannelInfo.m_ENCODER.m_Mosaic_y_coordinateLimitLow = 0;
					plChnNode->m_ChannelInfo.m_ENCODER.m_Mosaic_y_coordinateLimitHigh = 1080;

					plChnNode->m_ChannelInfo.m_ENCODER.m_Mosaic_Width_LimitLow = 8;
					plChnNode->m_ChannelInfo.m_ENCODER.m_Mosaic_Width_LimitHigh = 1920;

					plChnNode->m_ChannelInfo.m_ENCODER.m_Mosaic_Height_LimitLow = 8;
					plChnNode->m_ChannelInfo.m_ENCODER.m_Mosaic_Height_LimitHigh = 1080;
#endif
					...
					for (k = 0; k < plChnNode->m_SubChannelNumber; k++)
					{
						...
						plEncInfo->m_EncMode = ENC_MODE_H265;
#endif
						...
						plEncInfo->m_AudioGroup =  SDI_GRP1_CH_1_2;
...
#if defined(SUPPORT_WATER_MARK)
						plEncInfo->m_x_coordinate = 64;
						plEncInfo->m_y_coordinate = 64;
						plEncInfo->m_transparent = 50;
						plEncInfo->m_enable = FALSE;
#endif
#if defined(SUPPORT_MOSIAC)
						/* Mosaic*/
						plEncInfo->m_Mosaic_Param.m_x_coordinate = 64;
						plEncInfo->m_Mosaic_Param.m_y_coordinate = 64;
						plEncInfo->m_Mosaic_Param.m_width = 16;
						plEncInfo->m_Mosaic_Param.m_height = 16;
						plEncInfo->m_Mosaic_Param.m_blk_size = 0;
						plEncInfo->m_Mosaic_Param.m_enable = FALSE;
#endif
	...
	/*通道信息*/
	{
...
#if defined(SUPPORT_WATER_MARK)
					plXMLData = mxmlNewElement(plXMLChnNode, "x_coordinateLimitLow");
					mxmlNewTextf(plXMLData, 0, "%d", plChnNode->m_ChannelInfo.m_ENCODER.m_x_coordinateLimitLow);
					plXMLData = mxmlNewElement(plXMLChnNode, "x_coordinateLimitHigh");
					mxmlNewTextf(plXMLData, 0, "%d", plChnNode->m_ChannelInfo.m_ENCODER.m_x_coordinateLimitHigh);

					plXMLData = mxmlNewElement(plXMLChnNode, "y_coordinateLimitLow");
					mxmlNewTextf(plXMLData, 0, "%d", plChnNode->m_ChannelInfo.m_ENCODER.m_y_coordinateLimitLow);
					plXMLData = mxmlNewElement(plXMLChnNode, "y_coordinateLimitHigh");
					mxmlNewTextf(plXMLData, 0, "%d", plChnNode->m_ChannelInfo.m_ENCODER.m_y_coordinateLimitHigh);

					plXMLData = mxmlNewElement(plXMLChnNode, "transparentLimitLow");
					mxmlNewTextf(plXMLData, 0, "%d", plChnNode->m_ChannelInfo.m_ENCODER.m_transparentLimitLow);
					plXMLData = mxmlNewElement(plXMLChnNode, "transparentLimitHigh");
					mxmlNewTextf(plXMLData, 0, "%d", plChnNode->m_ChannelInfo.m_ENCODER.m_transparentLimitHigh);
#endif					

#if defined(SUPPORT_MOSIAC)
					/* Mosaic */
					plXMLData = mxmlNewElement(plXMLChnNode, "Mosaic_x_coordinateLimitLow");
					mxmlNewTextf(plXMLData, 0, "%d", plChnNode->m_ChannelInfo.m_ENCODER.m_Mosaic_x_coordinateLimitLow);
					plXMLData = mxmlNewElement(plXMLChnNode, "Mosaic_x_coordinateLimitHigh");
					mxmlNewTextf(plXMLData, 0, "%d", plChnNode->m_ChannelInfo.m_ENCODER.m_Mosaic_x_coordinateLimitHigh);

					plXMLData = mxmlNewElement(plXMLChnNode, "Mosaic_y_coordinateLimitLow");
					mxmlNewTextf(plXMLData, 0, "%d", plChnNode->m_ChannelInfo.m_ENCODER.m_Mosaic_y_coordinateLimitLow);
					plXMLData = mxmlNewElement(plXMLChnNode, "Mosaic_y_coordinateLimitHigh");
					mxmlNewTextf(plXMLData, 0, "%d", plChnNode->m_ChannelInfo.m_ENCODER.m_Mosaic_y_coordinateLimitHigh);

					plXMLData = mxmlNewElement(plXMLChnNode, "Mosaic_Width_LimitLow");
					mxmlNewTextf(plXMLData, 0, "%d", plChnNode->m_ChannelInfo.m_ENCODER.m_Mosaic_Width_LimitLow);
					plXMLData = mxmlNewElement(plXMLChnNode, "Mosaic_Width_LimitHigh");
					mxmlNewTextf(plXMLData, 0, "%d", plChnNode->m_ChannelInfo.m_ENCODER.m_Mosaic_Width_LimitHigh);

					plXMLData = mxmlNewElement(plXMLChnNode, "Mosaic_Height_LimitLow");
					mxmlNewTextf(plXMLData, 0, "%d", plChnNode->m_ChannelInfo.m_ENCODER.m_Mosaic_Height_LimitLow);
					plXMLData = mxmlNewElement(plXMLChnNode, "Mosaic_Height_LimitHigh");
					mxmlNewTextf(plXMLData, 0, "%d", plChnNode->m_ChannelInfo.m_ENCODER.m_Mosaic_Height_LimitHigh);
#endif					
			...
			for (k = 0; k < lTmpSubChnNum; k++)
			{
				...
				case HWL_CHANNEL_SUBTYPE_IP:
...
#if defined(GQ3655) || defined(GQ3710B) || defined(GQ3760A) || defined(GQ3760)
					...
					case HWL_CHANNEL_SUBTYPE_ENCODER_HI3519:
					{
						...
						MULTL_XMLAddNodeText(plXMLSubNode, "audio_group", MULTL_XMLAudGroupValueToStr(plSubNode->m_SubInfo.m_SubENCODERInfo.m_AudioGroup));
...
#if defined(SUPPORT_WATER_MARK)
						MULTL_XMLAddNodeUINT(plXMLSubNode, "x_coordinate", plSubNode->m_SubInfo.m_SubENCODERInfo.m_x_coordinate);
						MULTL_XMLAddNodeUINT(plXMLSubNode, "y_coordinate", plSubNode->m_SubInfo.m_SubENCODERInfo.m_y_coordinate);
						MULTL_XMLAddNodeUINT(plXMLSubNode, "transparent", plSubNode->m_SubInfo.m_SubENCODERInfo.m_transparent);
						MULTL_XMLAddNodeMark(plXMLSubNode, "enable", plSubNode->m_SubInfo.m_SubENCODERInfo.m_enable);
#endif
#if defined(SUPPORT_MOSIAC)
						/* Mosaic */
						MULTL_XMLAddNodeUINT(plXMLSubNode, "mosaic_x_coordinate", plSubNode->m_SubInfo.m_SubENCODERInfo.m_Mosaic_Param.m_x_coordinate);
						MULTL_XMLAddNodeUINT(plXMLSubNode, "mosaic_y_coordinate", plSubNode->m_SubInfo.m_SubENCODERInfo.m_Mosaic_Param.m_y_coordinate);
						MULTL_XMLAddNodeUINT(plXMLSubNode, "mosaic_width", plSubNode->m_SubInfo.m_SubENCODERInfo.m_Mosaic_Param.m_width);
						MULTL_XMLAddNodeUINT(plXMLSubNode, "mosaic_height", plSubNode->m_SubInfo.m_SubENCODERInfo.m_Mosaic_Param.m_height);
						MULTL_XMLAddNodeText(plXMLSubNode, "mosaic_size", MULTL_XMLEncMosaicSizeValueToStr(plSubNode->m_SubInfo.m_SubENCODERInfo.m_Mosaic_Param.m_blk_size));
						MULTL_XMLAddNodeMark(plXMLSubNode, "mosaic_enable", plSubNode->m_SubInfo.m_SubENCODERInfo.m_Mosaic_Param.m_enable);
#endif
                ...
                //GLOBAL_TRACE((" =====>>> Test: InputChannel[%d] | m_ActiveMark = %d \n", k, plSubNode->m_ActiveMark));
		...
		for (i = 0; i < plSystemParam->m_OutChannelNumber; i++)
		{
			...
			for (k = 0; k < plChnNode->m_SubChannelNumber; k++)
			{
                ...
                //GLOBAL_TRACE((" =====>>> Test: OutputChannel[%d] | m_ActiveMark = %d \n", k, plSubNode->m_ActiveMark));
				MULTL_XMLAddNodeMark(plXMLSubNode, "active_mark", plSubNode->m_ActiveMark);
			...
			if (plXML)
			{
				...
				if (plXMLRoot)
				{
					...
					if (plTmpStr)
					{
						...
						pHandle->m_AuthMode = MULTL_XMLGetNodeINT(plXMLRoot, "current_auth_signature", 10);
...
#if defined(GN1846) || defined(GN1866) || defined(GN1866B)
								/*修正 旧版本->3.5以上的时候输出IP通道信息会丢失的问题*/
								{
									S32 kk;
									MULT_ChannelNode *plENCNodeIN, *plENCNodeOUT;
									MULT_SubChannelNode *plENCSubNodeIN, *plENCSubNodeOUT;

									plENCNodeIN = &plSystemParam->m_pInChannel[0];
									plENCNodeOUT = &plSystemParam->m_pOutChannel[0];
									//GLOBAL_TRACE(("IN TYPE = %d, Sub = %d!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n", plENCNodeIN->m_ChannelType, plENCNodeOUT->m_SubChannelNumber));
									//GLOBAL_TRACE(("IN TYPE = %d, OUT TYPE = %d!!!!!!!!!!!!!!!!!!!!!!\n", plENCNodeOUT->m_ChannelType, plENCNodeOUT->m_SubChannelNumber));
									if (plENCNodeIN->m_ChannelType == HWL_CHANNEL_TYPE_ENCODER)
									{
										if (plENCNodeOUT->m_ChannelType == HWL_CHANNEL_TYPE_IP)
										{
											if (plENCNodeIN->m_SubChannelNumber == plENCNodeOUT->m_SubChannelNumber)
											{
												for (kk = 0; kk < plENCNodeIN->m_SubChannelNumber; kk++)
												{
													plENCSubNodeIN = &plENCNodeIN->m_pSubChannelNode[kk];
													plENCSubNodeOUT = &plENCNodeOUT->m_pSubChannelNode[kk];

													if (plENCSubNodeIN->m_SubInfo.m_SubENCODERInfo.m_IPv4Addr != 0)
													{
														plENCSubNodeOUT->m_SubInfo.m_SubIPInfo.m_IPv4Addr = plENCSubNodeIN->m_SubInfo.m_SubENCODERInfo.m_IPv4Addr;
														plENCSubNodeOUT->m_SubInfo.m_SubIPInfo.m_IPv4Port = plENCSubNodeIN->m_SubInfo.m_SubENCODERInfo.m_IPv4Port;
														plENCSubNodeOUT->m_SubInfo.m_SubIPInfo.m_Protocol = plENCSubNodeIN->m_SubInfo.m_SubENCODERInfo.m_Protocol;
														GLOBAL_TRACE(("Sub %d, IP = 0x%08X!!!!!!!!!!!!!!!!!!\n", kk, plENCSubNodeOUT->m_SubInfo.m_SubIPInfo.m_IPv4Addr));
														plENCSubNodeIN->m_SubInfo.m_SubENCODERInfo.m_IPv4Addr = 0;
													}
...
void MULTL_DefaultMonitorSetting(MULT_Handle *pHandle, HWL_HWInfo* pHWInfo)
{
	...
	CAL_LogSetUsedMark(plMonitor->m_LogHandle, MULT_MONITOR_TYPE_FPGA_HEART_BEAT, TRUE);
	...
	if(HWL_Get_BoardType() == INTERFACE_BOARD_SDI)
	{
		CAL_LogSetUsedMark(plMonitor->m_LogHandle, MULT_MONITOR_TYPE_AUDIO_IN_GROUP_INPUT_ERROR, TRUE);
	}
#else
	CAL_LogSetUsedMark(plMonitor->m_LogHandle, MULT_MONITOR_TYPE_TEMP, TRUE);
	CAL_LogSetUsedMark(plMonitor->m_LogHandle, MULT_MONITOR_TYPE_FPGA_HEART_BEAT, TRUE);
...
BOOL MULTL_LoadParamterFromStorage(MULT_Handle *pHandle, CHAR_T *pFilePathName)
{
						...
						if (CAL_FCAPValidation(plFCAHandle, MULT_DEVICE_COMPLETE_TYPE, MULT_PARAMETER_DESCRIPTOR))
						{
							...
							if (lWriteFD)
							{
								...
								if (lActSize == lFileSize)
								{
									...
									GLOBAL_TRACE(("Load Paramters From Storage Successful, Tar Size = %d\n", lFileSize, pFilePathName));
									lRet = TRUE;
								}
								else
								{
	...
	{
		GLOBAL_TRACE(("Copy OLD WM File!!!!!!!!!!\n"));
		/*复制旧的水印到新的版本中*/
		PFC_System("cd %s\ntar -xzf %s\n", MULT_XML_BASE_DIR, WATER_MARK_FILE_OLD_TAR);
		PFC_System("mv %s %s\n", WATER_MARK_FILE_OLD, WATER_MARK_FILE);
	}
	return lRet;
}
