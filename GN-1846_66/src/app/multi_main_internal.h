...
typedef struct
{
	...
	BOOL	m_LicenseValid;
#if defined(GN1846) || defined(GN1866)  || defined(GN1866B)
	S32		m_InputBoardType; /* 输入板子类型*/
	U8		m_LicenseAAC;/*AAC编码功能是否授权*/
	U8		m_LicenseAC3;/*AC3编码功能是否授权,此版本未使用，备用*/
#endif
...
// Timing Apply Parameter Start
	S32		m_TimingInterval;//更新间隔（秒） 默认值6*3600(6个小时)
	BOOL8	m_TimingMark;
...
typedef enum 
{
	...
	MULT_MONITOR_TYPE_AUDIO_IN_GROUP_INPUT_ERROR, /* 音频输入组选择错误 */
#endif
...
// 	BOOL8			m_OutputLink;//仅限IP等综合通道
	...
	//BOOL					m_Chn2SetParamFlag;//第二个通道配置参数Flag
	S32						m_MEMSize;//内存大小
	S32						m_CpuCurrentTemp;//cpu当前温度
...
/*封装函数*/
void MULT_FRPWarpPreInitate(MULT_Handle *pHandle);//系统刚刚初始化的时候调用，首先配置屏幕和LED灯
void MULT_FRPWarpPreInitateSetText(S32 Line, CHAR_T *pText);//在正式初始化之前可以设置文本
void MULT_FRPWarpMainInitate(MULT_Handle *pHandle);//系统初始化完成，进入正常工作状态
