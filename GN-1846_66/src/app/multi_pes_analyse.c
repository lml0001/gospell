...
void PESL_ModuleTaskFn(void *pUserParam)
{
	...
	if (plHandle)
	{
		...
		{
			lDuration = CAL_TimeDuration(&lTimeTick, PFC_GetTickCount(), 10);
...
BOOL PES_StartAnalyse(S32 TsInd, U32 ESPID, S32 ESType)
{
	...
	if (plHandle)
	{
		if (plHandle->m_State == PES_STATE_IDLE)
		{
			GLOBAL_TRACE(("Analyse PES Start TsInd = %d, ESType = %d(%s), ESPID = %d\n", TsInd, ESType, MPEG2_PES_VIDFREnumToStr(MPEG2_PES_MPEG2SYSAVFormatCodeToEnum(ESType)), ESPID));
...
BOOL PES_GetVIDEsInfoStr(CHAR_T *pBuf, S32 BufSize)
{
	...
	if (plHandle)
	{
		if ((plHandle->m_State == PES_STATE_IDLE) && (plHandle->m_CompleteMark))
		{
			...
			GLOBAL_SNPRINTF((pBuf, BufSize, "%s %dX%d%c@", MPEG2_PES_VIDFREnumToStr(MPEG2_PES_MPEG2SYSAVFormatCodeToEnum(plHandle->m_ESType)), \
