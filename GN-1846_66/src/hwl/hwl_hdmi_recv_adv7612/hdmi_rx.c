...
static void HDMI_RxEepromI2cDelay(S32 Count)
{
	...
	PFC_DelayNS(Count * 1000 * 100);//2019-4-24 薛晶修正EDID下载错误。问题是-O2优化之后的时序问题
}
...
HANDLE32 HDMI_RxCreate(HDMI_RxInitParam *pInitParam)
{
    ...
    GLOBAL_TRACE(("###########1111111##########\n"));
    while(1)
    {
        pInitParam->m_GpioSetValueCB(&pInitParam->m_Adv7612I2cSdaPin, LEVEL_LOW, pInitParam->m_pUserParam);
        PL_TaskSleep(100);
        pInitParam->m_GpioSetValueCB(&pInitParam->m_Adv7612I2cSdaPin, LEVEL_HIGH, pInitParam->m_pUserParam);
        PL_TaskSleep(100);
    }
...
#if defined(GN1866)
	/* 使用外部EDID，外同步 */
	adv7612_set_params(plHandle->m_Adv7612Handle, ADV7612_LLC_DLL, 0x81);
#endif
	/* 使能均衡功能 */
	{
		int regs[3];
...
//    PFC_SemaphoreWait(HDMI_RxGetLock, -1);
	HDMI_RxHandle *plHandle = (HDMI_RxHandle *)Handle;
...
BOOL HDMI_Probe(HANDLE32 Handle)
{
	int ret = 0;
	HDMI_RxHandle *plHandle = (HDMI_RxHandle *)Handle;
	S32 reg_addr = 0;
	...
	ret = adv7612_get_params(plHandle->m_Adv7612Handle, ADV7612_GET_REG, (S32)&reg_addr);
