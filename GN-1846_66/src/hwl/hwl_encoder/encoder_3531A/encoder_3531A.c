...
#define AUD_BUFF_SIZE (SAMPLE_AUDIO_PTNUMPERFRM * 4 * 10) /* 立体声左右声道 * 2, 每个采样点 16 位，所以字节数要乘 4 */
...
#else
	HANDLE32		m_SocketHandle;//去掉Socket接口，放到外侧
#endif
...
typedef struct {
	...
	BOOL					m_IpOutputTypeChangeFlag; /* MPTS/SPTS 是否切换，若有切换，需要重置所有 buffer 内容 */
...
static BOOL ENC_3531AEthOutSendData(HANDLE32 lSocketHandle, U32 DestIp, U16 DestPort, U8 *pData, S32 DataLen)
{
	...
	{ /* 设置网络参数的时候，需要将数据丢弃，否则会导致出现编码溢出和码流过大情况 */
		return FALSE;
	}
#endif
			...
			case AUD_PROC_START:  
				{
					...
					if (FD_ISSET(lAiFd, &lReadFds)) {
						...
						if((plAudProcParam->m_AudEncMode == AUD_ENC_MODE_AC3) ||
							...
							if (RngNBytes(lAc3Param.m_pAC3RngBuf) >= lAc3Param.m_Ac3FrameSize*15) {/*留十五帧数据（250-400ms左右）作为编码延时*/
