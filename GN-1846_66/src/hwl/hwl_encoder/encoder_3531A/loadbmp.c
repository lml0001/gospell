...
int LoadPNGCanvas(const char *filename, OSD_LOGO_T *pVideoLogo, OSD_COLOR_FMT_E enFmt)
{
    IMA_Param lIMAParam;
    HI_U16  i;
	 HI_U16 j;
    ...
    //add by leonli
    char *TempFileName = (char*)filename;

    //edit by leonli
    //if(NULL == filename)
    if(NULL == TempFileName)
    {
        printf("LoadPNGCanvas: filename=NULL\n");
        return -1;
    }
    
     GLOBAL_ZEROSTRUCT(lIMAParam);

	 lIMAParam.m_Type = IMA_TYPE_ARGB8888;

    //edit by leonli
    //if (IMA_WarpPNGDecodeFile(filename, &lIMAParam) == FALSE)
    if (IMA_WarpPNGDecodeFile(TempFileName, &lIMAParam) == FALSE)
    {
        printf("Read File Failed!!!!\r\n");
		 return -1;
    }

    if ((lIMAParam.m_pDataBuf == NULL) || (lIMAParam.m_ActSize <= 0))
    {
        printf("RGB File Load Failed!!!!!!!!!!\n");
	     return -1;
    }
 	
    w = (HI_U16)lIMAParam.m_Width;
    h = (HI_U16)((lIMAParam.m_Height>0)?lIMAParam.m_Height:(-lIMAParam.m_Height));
    Bpp = lIMAParam.m_PIXSize;
    ...
    /* RGB8888 or RGB1555 */
    pOrigPNGBuf = lIMAParam.m_pDataBuf ;
    if(NULL == pOrigPNGBuf)
    {
        printf("not enough memory to malloc!\n");
        return -1;
    ...
    for (i=0; i<h; i++)
    {
        ...
        {  
				/* start color convert */
				pStart = pOrigPNGBuf + i*stride + j*Bpp;

				/* VENC Support ARGB1555 is 16 bit.*/
				pDst = (HI_U16*)(pRGBBuf + i*pVideoLogo->stride + j*2);
				a = *(pStart); 
				b = *(pStart + 1);
				g = *(pStart + 2);
				...
				*pDst = OSD_MAKECOLOR_U16(r, g, b, s_OSDCompInfo[OSD_COLOR_FMT_RGB1555]);
    ...
    GLOBAL_SAFEFREE(lIMAParam.m_pDataBuf);
    return 0;
}
...
HI_S32 CreateSurfaceByBitMap(const HI_CHAR *pszFileName, OSD_SURFACE_S *pstSurface, HI_U8 *pu8Virt)
{
    ...
    stLogo.pRGBBuffer = pu8Virt;
    if(LoadPNGCanvas(pszFileName, &stLogo, pstSurface->enColorFmt) < 0)
    {
